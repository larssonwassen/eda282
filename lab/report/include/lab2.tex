...
In \reffig{fig:lab2} results from running the parallelized version of the program in the simulator with different CCP:s and different matrix dimensions are shown. It can be seen that using MSI or MESI does not cause any major change in performance but there is a small speed improvement with MESI over MSI at the bigger matrix sizes. This can be explained by the fact that it is cheaper to go to M state from E state than from S state as this would cause invalidations to be sent on the buss. 
